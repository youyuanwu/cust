/* cstd::alloc — pluggable allocator interface.
 *
 ...
 * allocators slotting under the same string / vec / hashmap
 * types we ship in later modules.
 *
 * Shape:
 *
 *   struct cstd_alloc {
 ...
 *
 * The vtable carries three slots — `allocate`, `deallocate`,
 * `reallocate` — and every slot takes `(size, align)` (and on
 * dealloc/realloc the *old* `size`). That symmetry is what
 * lets a bump or pool allocator actually do its job; libc's
 * `free()` ignores them, but anything fancier needs them, and
 * adding them later is an API break.
 *
 * Convention:

/* cstd::rc — single-threaded kref-shaped refcount.
 *
 ...
 * embed `struct cstd_rc` as a field in their own type and
 ...
 *   };
 *
 *   static void my_thing_release(struct cstd_rc *r) {
 ...
 *       if (t) cstd_rc_put(&t->rc, my_thing_release);
 *   }
 *
 ...
 * use `cstd::arc` — same API shape with `_Atomic` count and
...
struct [[cust::pub_repr]] cstd_rc {
    u32 count;
};

[[cust::pub]] void cstd_rc_init(struct cstd_rc *r) {
    r->count = 1;
}

[[cust::pub]] void cstd_rc_get(struct cstd_rc *r) {
    r->count++;
}
...
[[cust::pub]] bool cstd_rc_put(struct cstd_rc *r,
                               void (*release)(struct cstd_rc *)) {
    if (--r->count == 0) {
        release(r);
        return true;
    }
 ...
 * `cstd_rc_put` already returns. */
[[cust::pub]] u32 cstd_rc_count(const struct cstd_rc *r) {
    return r->count;
}
...
[[cust::test]] int test_rc_init_starts_at_one(void) {
    struct cstd_rc r;
    cstd_rc_init(&r);
    cust_assert_eq(cstd_rc_count(&r), (u32)1);
    return 0;
}
...
static void dummy_release(struct cstd_rc *r) {
    (void)r;
...
[[cust::test]] int test_rc_get_put_invokes_release_once(void) {
    ...
    cstd_rc_get(&r);
    cstd_rc_get(&r);
    cust_assert_eq(cstd_rc_count(&r), (u32)3);

    i32 before = g_dummy_release_count;
    cust_assert(!cstd_rc_put(&r, dummy_release));   /* 2 */
    cust_assert(!cstd_rc_put(&r, dummy_release));   /* 1 */
    cust_assert_eq(g_dummy_release_count, before);  /* not yet */
    cust_assert(cstd_rc_put(&r, dummy_release));    /* 0 — fires */
    cust_assert_eq(g_dummy_release_count, before + 1);
    return 0;
}

/* End-to-end: a consumer type that embeds `struct cstd_rc` and
...
#include <stddef.h>    /* offsetof — needed by the consumer
                          recovery pattern; cstd itself doesn't
                          use it */
...
static void rc_blob_release(struct cstd_rc *r) {
...
static bool rc_blob_put(struct rc_blob *b) {
    if (!b) return false;
    return cstd_rc_put(&b->rc, rc_blob_release);
}
...
[[cust::test]] int test_rc_end_to_end_lifecycle(void) {
    ...
    cust_assert(b == a && c == a);
    cust_assert_eq(cstd_rc_count(&a->rc), (u32)3);
    ...
    cust_assert_eq(g_rc_acct_live, before + 1);
    cust_assert_eq(cstd_rc_count(&a->rc), (u32)1);

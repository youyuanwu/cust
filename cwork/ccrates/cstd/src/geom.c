/* cstd::geom — v0.4.0 dogfood for the plugin v1 body-export
 * (V40D-4) capability. Defines a small `[[cust::pub_repr]]`
 * struct that consumers can construct by value plus a
 * function that takes two by-value `point`s — without the
 * body export, the consumer would only see an opaque tag and
 * couldn't construct the struct at all (the v0.3.x state).
 *
 ...
 * see the slice A debugging notes in v0.4.0.md). */

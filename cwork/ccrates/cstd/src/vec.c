/* cstd::vec — owned, growable, type-erased dynamic array.
 *
 * Same shape as `cstd_string`: allocator-by-value, fallible
 * grow, no hidden globals, `[[gnu::cleanup(cstd_vec_free)]]`
 * friendly.
 *
 ...
 *      is straightforward;
 *   -  no compile-time type check — `cstd_vec_push(&v, &x)`
 ...
 * site, e.g.
 *
 *     static inline bool push_i32(struct cstd_vec *v, i32 x) {
 *         return cstd_vec_push(v, &x);
 *     }
 *
 * No element destructor in v1: `pop` / `clear` / `free` only
 * release storage. If an element type owns heap (`cstd_string`,
 * file handle, …) the caller must drop it before discarding.
 * We will add an optional drop fnptr the first time a real
 * owning-element use case lands; speculatively adding it costs
 * a word per vec for no benefit today.
 */

#cust use crate::types;
#cust use crate::alloc;

struct [[cust::pub_repr]] cstd_vec {
    void              *data;        /* cap * elem_size bytes allocated;
                                       len * elem_size bytes used */
    usize              len;         /* element count */
    usize              cap;         /* element count */
    usize              elem_size;   /* bytes per element; may be 0 (ZST) */
    usize              elem_align;  /* power of two */
    struct cstd_alloc  alloc;
};

[[cust::pub]] struct cstd_vec cstd_vec_new_in(usize elem_size,
                                              usize elem_align,
                                              struct cstd_alloc a) {
    struct cstd_vec v;
    v.data       = (void *)0;
 ...
 * `need` if doubling would wrap. Mirrors `cstd_string`'s
 * `next_cap`; consider extracting to a shared internal helper
 * if a third user appears. */
...
[[cust::pub]] bool cstd_vec_reserve(struct cstd_vec *v, usize extra) {
...
[[cust::pub]] bool cstd_vec_with_capacity_in(struct cstd_vec *out,
                                             usize elem_size,
                                             usize elem_align,
                                             usize cap,
                                             struct cstd_alloc a) {
    *out = cstd_vec_new_in(elem_size, elem_align, a);
    if (cap == 0) return true;
    return cstd_vec_reserve(out, cap);
}

[[cust::pub]] void cstd_vec_free(struct cstd_vec *v) {
    if (!v || !v->data) return;
    usize bytes = 0;
...
[[cust::pub]] usize cstd_vec_len(const struct cstd_vec *v) {
    return v->len;
}

[[cust::pub]] usize cstd_vec_capacity(const struct cstd_vec *v) {
    return v->cap;
}
...
[[cust::pub]] bool cstd_vec_push(struct cstd_vec *v, const void *elem) {
    if (!cstd_vec_reserve(v, 1)) return false;
    if (v->elem_size != 0) {
 ...
 * release storage; pair with `cstd_vec_free` at end of life. */
[[cust::pub]] bool cstd_vec_pop(struct cstd_vec *v, void *out) {
...
[[cust::pub]] void *cstd_vec_get(struct cstd_vec *v, usize i) {
    if (i >= v->len) return (void *)0;
    if (v->elem_size == 0) return (void *)0;
    return (u8 *)v->data + i * v->elem_size;
}

[[cust::pub]] const void *cstd_vec_get_const(const struct cstd_vec *v,
                                             usize i) {
...
[[cust::pub]] void cstd_vec_clear(struct cstd_vec *v) {
    v->len = 0;
}
...
[[cust::test]] int test_vec_new_and_free(void) {
    struct cstd_vec v = cstd_vec_new_in(sizeof(i32), _Alignof(i32),
                                        cstd_alloc_system());
    cust_assert_eq(cstd_vec_len(&v), (usize)0);
    cust_assert_eq(cstd_vec_capacity(&v), (usize)0);
    cstd_vec_free(&v);
...
[[cust::test]] int test_vec_push_pop_i32(void) {
    ...
    cust_assert_eq(cstd_vec_len(&v), (usize)50);

    for (i32 i = 49; i >= 0; i--) {
        i32 out = -1;
        cust_assert(cstd_vec_pop(&v, &out));
     ...
     * also legal. */
    cust_assert(!cstd_vec_pop(&v, (void *)0));
    cstd_vec_free(&v);
    return 0;
}
...
[[cust::test]] int test_vec_get_indexes_correctly(void) {
    ...
    for (i32 i = 0; i < 10; i++) {
        i32 x = i * 7;
        cust_assert(cstd_vec_push(&v, &x));
    }
    for (usize i = 0; i < 10; i++) {
        i32 *p = (i32 *)cstd_vec_get(&v, i);
        cust_assert(p != (i32 *)0);
        cust_assert_eq(*p, (i32)(i * 7));
    }
    /* Out-of-bounds returns NULL. */
    cust_assert(cstd_vec_get(&v, 10) == (void *)0);
    cust_assert(cstd_vec_get(&v, (usize)-1) == (void *)0);
    cstd_vec_free(&v);
    return 0;
}
...
[[cust::test]] int test_vec_with_capacity_avoids_regrow(void) {
    struct cstd_vec v;
    cust_assert(cstd_vec_with_capacity_in(&v,
                                          sizeof(i32), _Alignof(i32),
                                          32,
                                          cstd_alloc_system()));
    cust_assert(cstd_vec_capacity(&v) >= (usize)32);

    usize cap_before  = cstd_vec_capacity(&v);
    void *data_before = v.data;
    for (i32 i = 0; i < 32; i++) {
        cust_assert(cstd_vec_push(&v, &i));
    }
    cust_assert_eq(cstd_vec_capacity(&v), cap_before);
...
[[cust::test]] int test_vec_clear_keeps_capacity(void) {
    struct cstd_vec v = cstd_vec_new_in(sizeof(i32), _Alignof(i32),
                                        cstd_alloc_system());
    for (i32 i = 0; i < 16; i++) cust_assert(cstd_vec_push(&v, &i));
    usize cap = cstd_vec_capacity(&v);
    cstd_vec_clear(&v);
    cust_assert_eq(cstd_vec_len(&v), (usize)0);
    cust_assert_eq(cstd_vec_capacity(&v), cap);
    cstd_vec_free(&v);
    return 0;
}
...
[[cust::test]] int test_vec_large_element_stride(void) {
    ...
    struct cstd_vec v = cstd_vec_new_in(sizeof(struct big),
                                        _Alignof(struct big),
                                        cstd_alloc_system());
    for (u64 i = 0; i < 20; i++) {
    ...
    for (usize i = 0; i < 20; i++) {
        struct big *p = (struct big *)cstd_vec_get(&v, i);
...
[[cust::test]] int test_vec_zst_bookkeeping(void) {
    struct cstd_vec v = cstd_vec_new_in(0, 1, cstd_alloc_system());
    for (i32 i = 0; i < 100; i++) {
        cust_assert(cstd_vec_push(&v, (void *)0));
    }
    cust_assert_eq(cstd_vec_len(&v), (usize)100);
    /* data must remain NULL — no allocation for ZSTs. */
    cust_assert(v.data == (void *)0);

    i32 sentinel = 42;
    for (i32 i = 0; i < 50; i++) {
        cust_assert(cstd_vec_pop(&v, &sentinel));
...
/* gnu::cleanup interop — same recipe as cstd_string. Uses an
...
[[cust::test]] int test_vec_gnu_cleanup_on_scope_exit(void) {
    ...
    {
        [[gnu::cleanup(cstd_vec_free)]]
        struct cstd_vec v = cstd_vec_new_in(sizeof(i32),
                                            _Alignof(i32),
                                            vacct_alloc());
        for (i32 i = 0; i < 20; i++) cust_assert(cstd_vec_push(&v, &i));

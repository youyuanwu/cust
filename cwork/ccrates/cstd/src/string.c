/* cstd::string — borrowed string view + owned growable string.
 *
 ...
 * indirection lives in `cstd_alloc.vtable`, so a `cstd_string`
 * backed by a bump arena uses *exactly* the same struct and
 * functions as one backed by libc.
 *
 ...
 *
 * Bytes, not NUL: storage is explicit `len`-counted. `push_byte(0)`
 * is legal. Trailing NUL is materialised on demand by
 * `cstd_string_as_cstr` for C interop only.
 */
...
/* ─── [[gnu::cleanup]] interop ────────────────────────────
 *
 * cstd doesn't ship a `cust_cleanup` macro (cstd has no macros),
 * so the user-facing recipe is to spell the GNU attribute at the
 ...
 *
 *   1. The attribute fires at scope exit (no leak).
 *   2. The attribute fires on early `return` mid-scope too.
 *
 * To prove the drop *actually ran* (and not just that the
 * attribute parsed), both tests run against a tiny accounting
 * allocator that wraps the system allocator and tracks net live
 * allocations in a static counter.
 */

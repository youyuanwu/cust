/* cstd::arc — thread-safe kref-shaped refcount.
 *
 * Atomic sibling of `cstd::rc` — same `init` / `get` / `put`
 ...
 *
 * Consumer pattern (mirrors `cstd::rc` exactly, just with this
 * struct in place of `struct cstd_rc`):
 *
 *   struct my_arc_thing {
 *       struct cstd_arc    arc;
 ...
 *   };
 *
 *   static void my_arc_thing_release(struct cstd_arc *a) {
...
struct [[cust::pub_repr]] cstd_arc {
    _Atomic u32 count;
};

[[cust::pub]] void cstd_arc_init(struct cstd_arc *a) {
...
[[cust::pub]] void cstd_arc_get(struct cstd_arc *a) {
...
[[cust::pub]] bool cstd_arc_put(struct cstd_arc *a,
                                void (*release)(struct cstd_arc *)) {
...
/* Snapshot read. As in `cstd::rc`, mostly for debugging/tests
 * — production code should not branch on this; the right
 * question is "did this `_put` destroy the object", which
 * `cstd_arc_put` already returns. Uses `Relaxed` because no
 * ordering with anything else is being established. */
[[cust::pub]] u32 cstd_arc_count(const struct cstd_arc *a) {
 ...
 * multi-thread harness, which cstd doesn't host today. The
...
[[cust::test]] int test_arc_init_starts_at_one(void) {
    struct cstd_arc a;
    cstd_arc_init(&a);
    cust_assert_eq(cstd_arc_count(&a), (u32)1);
    return 0;
}

static i32 g_arc_dummy_release_count = 0;
static void arc_dummy_release(struct cstd_arc *a) {
    (void)a;
...
[[cust::test]] int test_arc_get_put_invokes_release_once(void) {
    ...
    cstd_arc_get(&a);
    cstd_arc_get(&a);
    cust_assert_eq(cstd_arc_count(&a), (u32)3);

    i32 before = g_arc_dummy_release_count;
    cust_assert(!cstd_arc_put(&a, arc_dummy_release));   /* 2 */
    cust_assert(!cstd_arc_put(&a, arc_dummy_release));   /* 1 */
    cust_assert_eq(g_arc_dummy_release_count, before);
    cust_assert(cstd_arc_put(&a, arc_dummy_release));    /* 0 — fires */
...
static void arc_blob_release(struct cstd_arc *a) {
...
static bool arc_blob_put(struct arc_blob *b) {
    if (!b) return false;
    return cstd_arc_put(&b->arc, arc_blob_release);
}
...
[[cust::test]] int test_arc_end_to_end_lifecycle(void) {
    ...
    cust_assert(b == a && c == a);
    cust_assert_eq(cstd_arc_count(&a->arc), (u32)3);

    cust_assert(!arc_blob_put(b));
    cust_assert_eq(g_arc_acct_live, before + 1);
    cust_assert(!arc_blob_put(c));
    cust_assert_eq(g_arc_acct_live, before + 1);
    cust_assert_eq(cstd_arc_count(&a->arc), (u32)1);
